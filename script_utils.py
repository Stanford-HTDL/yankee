...
def tuple_to_args(fn: Callable):
    def tuple_to_args_wrapper(tup: tuple, **kwargs):
        if isinstance(tup, tuple):
            return fn(*tup, **kwargs)
        return fn(tup, **kwargs)
    return tuple_to_args_wrapper


def async_tuple_to_args(fn: Callable):
    async def tuple_to_args_wrapper(tup: tuple, **kwargs):
        if isinstance(tup, tuple):
            return await fn(*tup, **kwargs)
        return await fn(tup, **kwargs)
    return tuple_to_args_wrapper    
